...
file is at `0600` with its ACL entries masked to nothing WHEN THAT CHMOD TAKES — it is not verified
afterwards, and the helper that sets it says so: a leftover whose `fchmod` failed under a umask that
masks owner read stays at 0200, unreadable to its owner. Best effort, stated as best effort.
