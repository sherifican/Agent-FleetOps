...
3. **Make each action SPECIFIC + grounded** — name the actual file/config/component/operator-step, not "look into it." The deep understanding is what lets you write "upstream PR #NNNNN fixes `final_response` in the error path → compare against the local patch at <X>" instead of "a final_response PR appeared."
