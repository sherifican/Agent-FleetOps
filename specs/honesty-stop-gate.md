...
**Blocked, not unsent.** On a harness that keeps a blocked turn's prose in the transcript — the one
this gate was built on does — a block is not a retraction.
Correct or retract the assertion explicitly in the continuation; do not describe it as never delivered.
This is the prose counterpart of the *Claims written to files* limit below. See [producer and consumer vocabulary](the-silent-clear-problem.md#producer-and-consumer-vocabulary).
For probe validity, follow [Instrument self-defeat](../skills/guard-target-correctness/SKILL.md#instance-3--instrument-self-defeat).
...
1. Scans the agent's prose — and the bodies of the files it writes, see below — for **live-state claims** ("is running", "has completed", "in flight", "not yet started").
...
**Which part of a command counts.** A tool call is split into shell statements (`;`, `&`, `&&`, `||`, newline — quotes, escapes and `#` comments respected; `2>&1` is a redirect, not a separator) and each statement into pipeline stages. Subjects are credited only from a stage that *starts* with a verification command, plus the `grep`/`rg`/`awk`/`sed` filter stages after it — so `ps aux | grep deploy` credits `deploy` (even when the only match is the grep's own command line, because the gate reads the probe record, not the output), `cd /x && pgrep deploy` credits `deploy`, `pgrep a; pgrep b` credits both, and `pgrep a; echo b`, `pgrep a | tee b.log` and `pgrep a # b` credit only `a`. Whether a probe was VALID is the operator's check; apply the [Instance 3 controls](../skills/guard-target-correctness/SKILL.md#instance-3--instrument-self-defeat). That is the unit of observation: a probe of A must not vouch for B because B's name is somewhere in the same string. Not modelled, and documented rather than handled: a probe under a wrapper (`sudo pgrep`, `timeout 5 pgrep`, `bash -c 'pgrep …'`) or inside `$(…)`, backticks, a `(…)` subshell, a process substitution `<(…)`/`>(…)` or an array value `a=(…)` — including `x=$(…)` and a `|` inside the group — is not credited, so a claim resting on one blocks and the fix is to run the probe plainly; such a group is one word to the splitter and is blanked before the probe regex runs, so neither a `;` nor a `|` inside `$(true; pgrep x)` can produce a stage that credits `x`, and neither a `case` arm's `)` nor a `)` in a `#` comment inside the group ends it early (all of these did, until three publish-gate reviews caught them); a heredoc body is removed before the split — it is data handed to a command, and `pgrep -af deploy` quoted inside one is not a probe (measured 2026-09-21: before this it *was* credited, the opposite of what an earlier draft of this sentence claimed); `kill -0 <pid>` is not in the default verification list because it names a pid, not a subject. The splitter is total — it never raises — because a Stop hook that raises is fail-open.

**Claims written to files.** A claim the agent writes into a report, a handoff or a status file outlives the turn: whoever reads the file never sees the transcript, so the prose-only gate had a channel it could not see. Measured over the private transcript corpus this was built against — the corpus is not published, so the numbers are reported, not reproducible from this repository — heredoc writes of `.md` files outnumbered `Write`+`Edit` calls 1,998 to 728, and a replay of the same transcripts found 231 unbacked claims written through heredocs against 41 through the editing tools: the shell is the majority channel, not the corner case. So the gate scans file bodies exactly as it scans prose: the `content` of a `Write`, the `new_string` of an `Edit` (its `old_string` is the text being removed and is not scanned), and the body of a shell heredoc whose own pipeline stage is a sink that lands in a file — `cat > f <<EOF`, `> f cat <<EOF`, `tee f <<EOF > /dev/null`, `sudo tee f <<EOF`, `cat <<EOF | tee f`, in any statement position, with the tag written bare, quoted or backslash-quoted (`<<\EOF`). The closing tag must be the whole line, as the shell requires — an indented `  EOF` is body — so a claim cannot hide behind a fake terminator. Redirects belong to their own stage: `cat <<EOF | git commit -F - > log.txt` writes a commit message, not `log.txt`. The non-heredoc write forms are scanned too — `echo 'claim' > f`, `printf … >> f`, `echo 'claim' | tee f`, a here-string `cat <<< 'claim' > f` — with the command's unquoted arguments as the body, because two independent refutation legs each named that surface first. An unterminated heredoc is scanned: the shell warns and still writes the file. An interpreter's stdin (`python3 - <<PY`, even when the code writes a file), `git commit -m`, `| tee` after an interpreter, and a stream target (`/dev/stderr`, `/dev/null`, `-`) are not extracted; a wrapper the extractor does not know (`ionice` and `chronic` are known; a bespoke script is not) hides the sink, and that is the accepted residue. A body is scanned against what had been verified **at the moment of the write**: a probe in the same call has not returned yet, so `cat > r.md <<EOF … EOF; pgrep -af deploy` still blocks, and so does writing the claim and probing afterwards. Two classes of file are skipped by default and the skip list is configurable: source files by suffix (`.py`, `.ts`, `.tsx`, `.sh`, `.yaml`, …), where a claim phrase is a comment or a string, and a short list of files that legitimately *quote* claim phrases — this gate, its spec and skill, `CHANGELOG.*`, `HISTORY.*`. The report formats — `.md`, `.markdown`, `.mdx`, `.adoc`, `.txt`, `.rst`, `.log`, `.html`, `.json`, `.csv` — cannot be put on the suffix skip list; a record that must carry another party's claim quotes it, in double quotes, backticks or a blockquote, and quoted text is not scanned (single quotes are not stripped). A config that narrows the scan — an empty tool map, a tool with no body-field list, an empty path-key list, a report suffix, an entry that is not a suffix (`""`, `md`), a nameless glob, a cap outside 32 KiB–1 MiB — is refused at load and `--check-config` prints each refusal by name, so a narrowing cannot read as configured. One narrowing no loader can see: a `write_tools` or `write_path_keys` map whose names do not match the harness's tool input turns the file scan off while reading as configured, so `--check-config` prints a notice whenever either differs from the default, and the proof is end-to-end — a `Write` carrying an unbacked claim must block. An exception while scanning a body is a `CANNOT CHECK` block naming the path: the hook does not fail closed on a crash, and an unscanned body is not a clean one. A skip pattern must name something — a literal basename (a dotfile such as `.env` counts) or, for a glob basename, at least two literal directory segments (`skills/honesty-stop-gate/*` names a place; `/home/*` and `tmp/*` name a tree and are refused); `*`, `*.md`, `**/*`, `*/*`, `.*` and bracket tricks like `[!.]*` are refused at load and dropped, because each would turn the file scan off while reading as configured. A body over 64 KiB (floor 32 KiB, and a per-turn total cap) is **not** scanned as a prefix — a prefix scan reads as coverage of the whole file — the turn blocks with `CANNOT CHECK` naming the path, and the fix is to split the write or keep the claims in prose. The block message for a file claim names the path and says what the three exits mean there: the file still says it, so deleting or relabelling the claim means rewriting the file; a blocked turn does not undo a write, and after one block the next Stop is not inspected, so the rewrite has to happen in the continuation. When the same call that wrote the file also ran the probe, the message says so — the probe's result was not back when the file was written; write the file after it. Cost: a body is scanned at the same per-claim cost as prose, and that cost grows with claims × body length. Two bounds hold it: the scan stops after eight findings in one body (the verdict is already a block), and it examines at most sixty-four claim phrases in one file body — beyond that the body is a `CANNOT CHECK` block, because a report with more claim phrases than that is not a report anyone reads (prose is not capped this way: a verified seventy-line status list in the turn's own text is scanned in full), and because the eight-finding bound alone does nothing when the claims are all backed. Measured before either bound, 64 KiB of unpunctuated claims held the hook for 102 s, and 64 KiB of backed padding around seven unbacked claims for 64 s (a hook that times out is fail-open); with both, each is under two seconds. One more thing this closed: a heredoc body used to be split into statements like the rest of the command, so a report that quoted `pgrep -af deploy` credited `deploy` — a body is data, and it is now removed before the split. The extractor was refuted three times before it shipped — two isolated legs with the code inlined and no premise, then a gate audit of the reworked bytes — and the self-test carries the findings as arms proven red on the candidate they broke: the `A-` arms (a backslash-quoted tag, a redirect before the command, an indented terminator taken as the closing tag, `tee f > /dev/null`, `sudo tee`, a commented `# <<EOF` swallowing a real probe, a stage's redirect attributed to another stage, two fail-open crash shapes), the `G-` arms (the `echo`/`printf`/here-string forms, `if …; then cat`, `>|`, a tag with a space, an unterminated body, the second heredoc on a line, `dd of=`, `$((1 << n))` and `<<<"…"` misread as openers, `let x=1; cat > f <<EOF` on one line hiding a heredoc, a quoted `--help`, `> -`, `tee >(proc)`, `.tsx`), `F-1`, the timing bound, and the `P-` arms, from the publish gate itself: the second heredoc on a line was extracted but not stripped, so a probe quoted inside it still credited (`P-1`); `;` inside a group started a new statement (`P-2`); a `|` inside a group was a stage break and `x=$(true; pgrep …)` read as an assignment followed by a probe (`P-3`); the finding bound alone left backed padding unbounded (`P-4`); a `let` at the start of a line hid every heredoc after it (`P-5`); `/home/*` passed the skip-pattern check (`P-6`); from the third review: the claim cap also applied to prose (`P-12`), a `)` inside a `#` comment closed the group (`P-11`), `case` as an argument and `${x:=(}` false-blocked (`P-13`), a 60 KB token cost the filename regex 22 s (`P-14`); and from the grok re-check of the fix: `<(…)`/`>(…)` were not groups (`P-7`), a `case` arm's `)` closed the group early (`P-8`), `arr=(…)` was not a group (`P-9`), a group with `|` on a heredoc opener line credited (`P-10`).
...
flowchart TD
    ...
    D --> E["Find live-state claims<br/>in the agent's prose and<br/>in the files it writes"]
