...
| Guard unit gates | `pytest guard/tests/ -q` | 661 tests, hermetic (one is environment-gated: it skips without `PASSBACK_OUTBOX`; the strict xfail that once recorded the scanner's stale-report gap was repaired in `3a9b87a` and is now a live arm) |
