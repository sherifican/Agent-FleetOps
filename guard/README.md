...
| Guard unit gates | `pytest guard/tests/ -q` | 1013 tests, hermetic in the sense that no arm needs a live fleet, a network or a credential. Measured on this box — Linux, `setfacl` present, unprivileged — 1012 pass and one skips (a clone without the private identity list skips one more: the arm that reports which identity terms the publication gate read): the passback arm, without `PASSBACK_OUTBOX`. That is an observation of one machine, not a property of the suite: several arms stand down where their instrument is missing, and each of these moves the number — no `O_TMPFILE`, a filesystem that rejects it, running as root, a filesystem refusing a default ACL, or `setfacl` declining to strip one. A skipped arm is not a passing one, so read `-rs` for what stood down on YOUR machine rather than trusting the count here. The strict xfail that once recorded the scanner's stale-report gap was repaired in `a0dbe05` and is now a live arm |
