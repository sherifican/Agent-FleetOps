...
| Guard unit gates | `pytest guard/tests/ -q` | 695 tests, hermetic in the sense that no arm needs a live fleet, a network or a credential. On an ordinary Linux box with `setfacl` present, exactly one skips: the passback arm, without `PASSBACK_OUTBOX`. Several others carry environment gates that stand them down where their instrument is missing — no `O_TMPFILE`, running as root, or a filesystem refusing a default ACL — which is why a skipped arm is not a passing one and `-rs` is worth reading rather than assuming. The strict xfail that once recorded the scanner's stale-report gap was repaired in `3a9b87a` and is now a live arm |
