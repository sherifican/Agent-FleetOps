...
Each guard was written after its incident, and nothing tests that any of them still works. That
is how you get a check that never fires — not by writing a bad check, but by writing a good check whose
input shape later moved. A `grep -c … || echo 0` monitor was silently inert for its entire life. A
