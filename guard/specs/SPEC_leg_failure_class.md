...
The dispatch loop (`dispatch_video_research.sh:109-115`) is `for attempt in 1 2` with **no delay and
...
stop*. Only that decision rule is adopted. Its header-driven timing and its 2s–30s
exponential backoff were rejected: the subprocess wrappers have no access to HTTP headers, and the
