...
clear **at least two of the three**:
  ...
  **(c) REAL CAPTURED MEDIA** — ≥3 curated frames from the adopting team's vision pipeline, or ≥3 fetched
       documents/pages the adopting team actually retrieved.
...
2. **Provenance block** (machine-filled, never hand-written): legs used + their byte counts ·
   ...
   this report met**.
3. **VERDICT** — ≤120 words. What this is, and what the adopting team should do. If the honest verdict is "nothing
...
7. **EXISTING PRACTICE** — placement against the adopting team's stack. Prevents redundant adoption.
...
- **Per-claim anchors** (`[MM:SS]`, fetched URL, `path:line`) are required in two cases and optional
  otherwise:
    (i)  the claim is **contested** — the legs disagreed, or it contradicts the adopting team's assumptions;
...
- **MUST-NOT-CITE as fact**: the adopting team's inference, an opinion, or a projection. Label those `[inference]`.
- **`⚠️ UNVERIFIED`** on anything the adopting team did not directly confirm — and name the next check.
...
- Images must be **frames the adopting team's vision pipeline captured** or **pages the adopting team fetched**. An OpenGraph/marketing
...
- **★ THE RULE THAT MATTERS (owner, 2026-07-31): EVERY SCREENSHOT MUST CARRY ITS RELEVANT DATA POINT.**
  ...
  **A frame that does not evidence a data point is padding and must be dropped, however good it looks.**
  This is the difference between illustration and evidence, and it is the reason to capture real
